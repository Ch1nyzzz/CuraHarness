...
1. Read `src/curaharness/optimizer.py` and follow the outer loop end-to-end;
...
Implemented in `LocomoOptimizer.run()` (`src/curaharness/optimizer.py:152-284`).
...
`src/curaharness/proposer_prompt.py`). The base prompt is identical across
...
(`src/curaharness/optimizer.py:220-221`).
...
`_bandit_core_files` (`src/curaharness/optimizer.py:1951`) keeps a fixed set of
