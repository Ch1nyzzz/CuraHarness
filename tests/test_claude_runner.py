...
def test_extract_tool_access_tracks_read_and_grep_requests():
                        ...
                        "content": [
                            {
                                ...
                                "id": "toolu_read_1",
                                "name": "Read",
                                "input": {"file_path": "/repo/src/curaharness/optimizer.py"},
                            },
                            {
                                "type": "tool_use",
                                "id": "toolu_grep_1",
    ...
    assert access["read_files"] == ["/repo/src/curaharness/optimizer.py"]
...
def test_run_codex_prompt_uses_absolute_cd_and_records_usage(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("curaharness.claude_runner.shutil.which", fake_which)
    monkeypatch.setattr("curaharness.claude_runner.subprocess.run", fake_run)
