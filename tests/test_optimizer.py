...
from curaharness.benchmark_workspaces import (
    LOCOMO_WORKSPACE_SPEC,
    MINIMAL_BENCHMARK_PACKAGE_INIT,
)
from curaharness import optimizer as optimizer_module
from curaharness.locomo_optimizer import LocomoOptimizer, LocomoOptimizerConfig
from curaharness.optimizer import _single_top_k
from curaharness.schemas import CandidateResult
import pytest
...
def test_default_proposer_evaluates_only_one_candidate(tmp_path, monkeypatch):
    ...
    assert "CuraHarness Proposer" in captured["prompt"]
    ...
    assert (snapshot / "project_source" / "src" / "curaharness" / "dynamic.py").exists()
    assert (snapshot / "project_source" / "src" / "curaharness" / "scaffolds" / "base.py").exists()
    assert not (snapshot / "project_source" / "src" / "curaharness" / "optimizer.py").exists()
...
def test_candidate_code_policy_rejects_runtime_trace_and_scorer_access(tmp_path):
    ...
    (generated / "bad_candidate.py").write_text(
        "\n".join(
            [
                "from curaharness.metrics import score_prediction",
                "",
                "def leak():",
...
def test_candidate_code_policy_rejects_raw_locomo_access_in_source_snapshot(tmp_path):
    ...
    scaffold_dir = project_source / "src" / "curaharness" / "scaffolds"
...
def test_source_candidate_policy_allows_preexisting_scorer_import(tmp_path):
    ...
    for root in (candidate_source, original_source):
        scaffold_dir = root / "src" / "curaharness" / "scaffolds"
        scaffold_dir.mkdir(parents=True)
        (scaffold_dir / "base.py").write_text(
            "from curaharness.metrics import retrieval_oracle_prediction\n",
            encoding="utf-8",
        )
    optimizer = LocomoOptimizer(LocomoOptimizerConfig(run_id="r", out_dir=tmp_path))
...
def test_source_candidate_policy_rejects_new_scorer_import(tmp_path):
    ...
    (candidate_source / "src" / "curaharness" / "scaffolds" / "memgpt_scaffold.py").write_text(
        "from curaharness.metrics import retrieval_oracle_prediction\n",
        encoding="utf-8",
    )
    optimizer = LocomoOptimizer(LocomoOptimizerConfig(run_id="r", out_dir=tmp_path))
    ...
    assert any(item["marker"] == "curaharness.metrics" for item in violations)
...
def test_optimizer_copies_declared_locomo_source_scope(tmp_path):
    ...
    assert (dest / "project_source" / "src" / "curaharness" / "dynamic.py").exists()
    assert (
        dest / "project_source" / "src" / "curaharness" / "__init__.py"
    ).read_text(encoding="utf-8") == MINIMAL_BENCHMARK_PACKAGE_INIT
    assert (dest / "project_source" / "src" / "curaharness" / "scaffolds" / "mem0_scaffold.py").exists()
    assert not (dest / "project_source" / "src" / "curaharness" / "optimizer.py").exists()
    assert not (dest / "project_source" / "src" / "curaharness" / "baseline.py").exists()
    assert not (dest / "project_source" / "src" / "curaharness" / "tau_banking.py").exists()
    assert not (dest / "project_source" / "src" / "curaharness" / "tau_agents").exists()
    assert not (dest / "project_source" / "src" / "curaharness" / "text_classification.py").exists()
...
def test_bandit_state_updates_file_rewards_and_policy_scores(tmp_path):
        ...
        json.dumps(
            {
                "files_read": {
                    ...
                    "source_snapshot/candidate/project_source/src/curaharness/scaffolds/memgpt_scaffold.py": {
                        "reads": 2,
                        "lines": 220,
                    },
                },
    ...
    (call_dir / "diff.patch").write_text(
        "diff --git a/src/curaharness/scaffolds/memgpt_scaffold.py "
        "b/src/curaharness/scaffolds/memgpt_scaffold.py\n",
        encoding="utf-8",
    )
...
def test_bandit_iters_from_policy_paths_extracts_reference_iterations(tmp_path):
    optimizer = LocomoOptimizer(LocomoOptimizerConfig(run_id="r", out_dir=tmp_path))
    paths = [
        "source_snapshot/candidate/project_source/src/curaharness/model.py",
...
def test_bandit_state_normalizes_diff_host_paths(tmp_path):
    ...
    (call_dir / "agent" / "tool_access.json").write_text(
        json.dumps(
            {
                "files_read": {
                    "/workspace/source_snapshot/candidate/project_source/src/curaharness/scaffolds/memgpt_scaffold.py": {
                        "reads": 1,
                        "lines": 220,
                    },
                },
    ...
    (call_dir / "diff.patch").write_text(
        "diff --git a/runs/r/proposer_calls/iter_001/source_snapshot/candidate/original_project_source/src/curaharness/scaffolds/memgpt_scaffold.py "
        "b/runs/r/proposer_calls/iter_001/source_snapshot/candidate/project_source/src/curaharness/scaffolds/memgpt_scaffold.py\n",
        encoding="utf-8",
    )
    ...
    target_key = "source_snapshot/candidate/project_source/src/curaharness/scaffolds/memgpt_scaffold.py"
...
def test_bandit_state_ignores_non_path_shell_arguments(tmp_path):
        ...
        json.dumps(
            {
                "files_read": {
                    ...
                    "src/curaharness/scaffolds/memgpt_scaffold.py": {
                        "reads": 1,
                        "lines": 220,
                    },
                    "curaharness/scaffolds/memgpt_scaffold.py": {
                        "reads": 1,
                        "lines": 220,
                    },
    ...
    assert not any(path.startswith("src/curaharness/") for path in state["files"])
    assert not any(path.startswith("curaharness/") for path in state["files"])
...
def test_optimizer_records_and_aggregates_proposer_metrics(tmp_path):
    ...
    result = SimpleNamespace(
        ...
        tool_access={
            "files_read": {"src/curaharness/optimizer.py": {"reads": 2, "lines": 42}},
    ...
    assert row["files_read"] == {"src/curaharness/optimizer.py": {"reads": 2, "lines": 42}}
