...
from curaharness.scaffolds import build_scaffold
from curaharness.scaffolds.base import MemoryScaffold
...
SOURCE_SCAFFOLD_CLASSES = {
    "bm25": ("curaharness.scaffolds.bm25_scaffold", "RankBM25Scaffold"),
    "mem0_source": ("curaharness.scaffolds.mem0_scaffold", "Mem0SourceScaffold"),
    "memgpt_source": ("curaharness.scaffolds.memgpt_scaffold", "MemGPTSourceScaffold"),
    "membank_source": ("curaharness.scaffolds.membank_scaffold", "MemoryBankSourceScaffold"),
}
...
def load_candidate_scaffold(candidate: dict[str, Any], *, project_root: Path) -> MemoryScaffold:
    ...
    context = (
        _isolated_curaharness_project(source_project_path)
        if source_project_path is not None
        else nullcontext()
    )
    with context:
        if module_path:
            ...
            if candidate_root is not None and module_name.startswith("curaharness.generated."):
                module_name = module_name.removeprefix("curaharness.generated.")
...
def _source_project_src_root(path: Path) -> Path:
    ...
    for item in candidates:
        if (item / "curaharness").is_dir():
            return item
    raise FileNotFoundError(
        f"source project path must contain curaharness package or project_source/src/curaharness: {path}"
    )
...
def _load_source_project_scaffold(
    ...
    *,
    ...
    with _isolated_curaharness_project(source_project_path):
        importlib.invalidate_caches()
        module = importlib.import_module(module_name)
        cls = getattr(module, class_name)
        scaffold = cls()
...
@contextmanager
def _isolated_curaharness_project(src_root: Path) -> Iterator[None]:
    """Temporarily import curaharness modules from a copied project source tree."""

    saved_modules = {
        name: module
        for name, module in list(sys.modules.items())
        if name == "curaharness" or name.startswith("curaharness.")
    }
    ...
    try:
        ...
        for name in [
            item for item in list(sys.modules) if item == "curaharness" or item.startswith("curaharness.")
        ]:
            sys.modules.pop(name, None)
...
def _load_module_path(module_path: str, *, project_root: Path) -> object:
    ...
    module_name = f"_curaharness_candidate_{abs(hash(path.resolve()))}"
