...
def project_root() -> Path:
    """Return the CuraHarness repository root."""
...
def prepare_locomo(
    *,
    ...
    """Materialize LOCOMO under CuraHarness and write deterministic splits."""

    dest = dest or default_data_path()
    source = source or _local_locomo_cache()
...
def _local_locomo_cache() -> Path | None:
    candidate = Path.home() / ".cache" / "curaharness" / "locomo10.json"
    if candidate.exists():
        return candidate
    return None
