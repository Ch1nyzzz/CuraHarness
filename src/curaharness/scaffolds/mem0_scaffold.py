...
from curaharness.source_base import PROJECT_ROOT, build_fingerprint, source_base_sample_dir, validate_source_base
from curaharness.upstream import load_mem0_memory_class
...
def _mem0_config(
    ...
    temp_dir = None if persist_dir else tempfile.TemporaryDirectory(prefix="curaharness_mem0_")
    base_dir = str(persist_dir or (temp_dir.name if temp_dir is not None else ""))
    collection = str(
        config.extra.get("collection_name")
        or re.sub(r"[^A-Za-z0-9_]", "_", f"curaharness_{example.sample_id}_{example.task_id}_{uuid.uuid4().hex[:8]}")
    )
    ...
    vector_store_config.update(
        {
            ...
            "collection_name": str(
                config.extra.get("collection_name")
                or re.sub(r"[^A-Za-z0-9_]", "_", f"curaharness_{example.sample_id}")
            ),
        }
    )
    mem0_config: dict[str, Any] = {
...
def _mem0_build_fingerprint(config: ScaffoldConfig) -> str:
    ...
    return build_fingerprint(
        ...
        extra=config.extra,
        logic_paths=[
            PROJECT_ROOT / "src" / "curaharness" / "scaffolds" / "mem0_scaffold.py",
            PROJECT_ROOT / "src" / "curaharness" / "upstream.py",
            *mem0_logic_paths,
        ],
    )
