"""Claude-proposer optimization loop for CuraHarness."""
...
from curaharness.dynamic import load_candidate_scaffold
from curaharness.evaluation import EvaluationRunner, run_initial_frontier
from curaharness.locomo import default_data_path, load_locomo_examples, prepare_locomo, select_split
from curaharness.model import DEFAULT_BASE_URL, DEFAULT_MODEL
from curaharness.optimization_cells import get_target_cells
from curaharness.pareto import ParetoPoint, pareto_frontier, save_frontier
from curaharness.post_eval import write_diff_digest, write_post_eval_artifacts
from curaharness.proposer_prompt import build_progressive_proposer_prompt
from curaharness.scaffolds import DEFAULT_EVOLUTION_SEED_SCAFFOLDS, DEFAULT_SCAFFOLD_TOP_KS
from curaharness.scaffolds.base import ScaffoldConfig
from curaharness.schemas import CandidateResult, LocomoExample
...
class LocomoOptimizer:
    ...
    def _candidate_test_spec(self, candidate: CandidateResult) -> dict[str, Any]:
        ...
        has_source_project = any(
            spec.get(key)
            for key in ("source_project_path", "project_source_path", "curaharness_source_path")
        )
    ...
    def _candidate_code_policy_violations(self, candidate: Any) -> list[dict[str, str]]:
        ...
        forbidden = {
            ...
            "score_prediction": "runtime code must not call CuraHarness scoring helpers",
            "curaharness.metrics": "runtime code must not import CuraHarness scoring helpers",
    ...
    def _candidate_policy_scan_paths(self, candidate: dict[str, Any]) -> list[Path]:
        ...
        source_project = self._candidate_source_project_root(candidate)
        if source_project is not None:
            package_dir = source_project / "src" / "curaharness"
    ...
    def _bandit_core_files(self) -> list[str]:
        ...
        project_paths = [
            f"source_snapshot/candidate/project_source/src/curaharness/{rel}"
    ...
    def _build_source_snapshot_workspace(
        ...
        *,
        ...
        self._copy_if_exists(
            self.project_root / "src" / "curaharness" / "scaffolds" / "base.py",
            candidate_dir / "base.py",
        )
        self._copy_if_exists(
            self.project_root / "src" / "curaharness" / "schemas.py",
        ...
        (candidate_dir / "SNAPSHOT.md").write_text(
            "\n".join(
                [
                    ...
                    "`project_source/src/curaharness` and relevant upstream source",
    ...
    def _copy_project_source_context(self, dest_dir: Path) -> None:
        source_pkg = dest_dir / "project_source" / "src" / "curaharness"
    ...
    def _source_scaffold_path(self, source_family: str) -> Path | None:
        ...
        if not name:
            return None
        return self.project_root / "src" / "curaharness" / "scaffolds" / name
